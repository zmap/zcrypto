package ocsp_test

import (
	"encoding/hex"
	"testing"

	"github.com/zmap/zcrypto/x509"
	"github.com/zmap/zcrypto/x509/revocation/ocsp"
)

const example_cert_with_ocsp_delegation = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

const revoked_cert = `
-----BEGIN CERTIFICATE-----

g7YS
-----END CERTIFICATE-----`

const issuer_cert = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE----- `

const ocsp_request_hex = `3051304f304d304b3049300906052b0e03021a05000414d1b1648b8c9f0dd16ba38acd2b5017d5f9cfc06404145f60cf619055df8443148a602ab2f57af44318ef021040516923bfde812659b57646b0694e24`

const ocsp_response_hex = ``

const ocsp_revoked_hex = ``

func parseCertPEM(t *testing.T) (cert *x509.Certificate, revoked *x509.Certificate, issuer *x509.Certificate) {
	certPool := x509.NewCertPool()
	ok := certPool.AppendCertsFromPEM([]byte(example_cert_with_ocsp_delegation))
	if !ok {
		t.Fail()
	}
	ok = certPool.AppendCertsFromPEM([]byte(issuer_cert))
	if !ok {
		t.Fail()
	}
	ok = certPool.AppendCertsFromPEM([]byte(revoked_cert))
	if !ok {
		t.Fail()
	}
	cert = certPool.Certificates()[0]
	issuer = certPool.Certificates()[1]
	revoked = certPool.Certificates()[2]
	return
}

func TestOCSPCreateRequest(t *testing.T) {
	cert, _, issuer := parseCertPEM(t)
	issuerKeyHash, err := ocsp.GetKeyHashSHA1(issuer)
	if err != nil {
		t.Error(err.Error())
	}
	issuerNameHash := ocsp.GetNameHashSHA1(issuer)
	requestBytes, err := ocsp.CreateRequest(cert, issuerKeyHash, issuerNameHash)
	if err != nil {
		t.Error(err.Error())
	}
	if hex.EncodeToString(requestBytes) != ocsp_request_hex {
		t.Fail()
	}
}

func TestOCSPParse(t *testing.T) {
	cert, _, issuer := parseCertPEM(t)
	respBytes, err := hex.DecodeString(ocsp_response_hex)
	if err != nil {
		t.Error(err.Error())
	}
	_, err = ocsp.ParseResponseForCert(respBytes, cert, issuer)
	if err != nil {
		t.Error(err.Error())
	}
}

func TestOCSPWithMismatchedResponse(t *testing.T) {
	_, revoked, issuer := parseCertPEM(t)
	respBytes, err := hex.DecodeString(ocsp_response_hex)
	if err != nil {
		t.Error(err.Error())
	}
	_, err = ocsp.ParseResponseForCert(respBytes, revoked, issuer) // this OCSP response is for the wrong cert,
	if err == nil {                                                // should fail because no matching cert in respBytes
		t.Fail()
	}
}

func TestOCSPForRevoked(t *testing.T) {
	_, revoked, issuer := parseCertPEM(t)
	respBytes, err := hex.DecodeString(ocsp_revoked_hex)
	if err != nil {
		t.Error(err.Error())
	}
	respParsed, err := ocsp.ParseResponseForCert(respBytes, revoked, issuer) // this OCSP response is for the wrong cert,
	if err != nil {                                                          // should fail because no matching cert in respBytes
		t.Error(err.Error())
	}
	if respParsed.CertificateStatus != "Revoked" || respParsed.IsRevoked != true {
		t.Fail()
	}
}

func TestStatusCode(t *testing.T) {
	var status ocsp.ResponseStatus
	status = 0
	if status.String() != "success" {
		t.Fail()
	}
	respError := &ocsp.ResponseError{
		Status: status,
	}
	if respError.Error() == "" {
		t.Fail()
	}
	statusJson, err := status.MarshalJSON()
	if err != nil {
		t.Error(err.Error())
	}
	// test good unmarshal
	err = status.UnmarshalJSON(statusJson)
	if err != nil {
		t.Error(err.Error())
	}
	// test bad unmarshal
	err = status.UnmarshalJSON(statusJson[0:5]) // this should fail!
	if err == nil {
		t.Fail()
	}
}

func TestBadIssuerHash(t *testing.T) {
	_, _, issuer := parseCertPEM(t)
	issuer.RawSubjectPublicKeyInfo = issuer.RawSubjectPublicKeyInfo[0:5] // mess up issuer key
	_, err := ocsp.GetKeyHashSHA1(issuer)                                // this should fail when unmarshaling key
	if err == nil {
		t.Fail()
	}
}
