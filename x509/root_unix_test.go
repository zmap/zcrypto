...
func TestEnvVars(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if r.Size() > len(tc.cns) {
				t.Errorf("got %v certs, which is more than %v wanted", r.Size(), len(tc.cns))
			}
		})
	}
}
...
func certPoolEqual(a, b *CertPool) bool {
	...
	if !reflect.DeepEqual(a.byName, b.byName) ||
		len(a.certs) != len(b.certs) {
		return false
	}
	for i := range a.certs {
		ca, cb := a.certs[i], b.certs[i]
		if !bytes.Equal(ca.Raw, cb.Raw) {
			return false
		}
	}
